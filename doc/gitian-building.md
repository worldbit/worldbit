...
- [Building Dash Core](#building-worldbit-core)
...
git clone https://github.com/worldbit/worldbit
```
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/worldbit/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/worldbit/worldbit
...
For example:
```bash
URL=https://github.com/crowning-/worldbit.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit worldbit=${COMMIT} --url worldbit=${URL} ../worldbit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit worldbit=${COMMIT} --url worldbit=${URL} ../worldbit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit worldbit=${COMMIT} --url worldbit=${URL} ../worldbit/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the worldbit git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../worldbit/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/worldbit.git
SIGPATH=/some/root/path/worldbit-detached-sigs.git

./bin/gbuild --url worldbit=${BTCPATH},signature=${SIGPATH} ../worldbit/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/worldbit-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/worldbit-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/worldbit-osx-build.assert
```
...
[worldbit/gitian.sigs](https://github.com/worldbit/gitian.sigs/) repository, or if that's not possible to create a pull
request.
