...
const char * const BITCOIN_CONF_FILENAME = "worldbit.conf";
const char * const BITCOIN_PID_FILENAME = "worldbitd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "worldbit" is a composite category enabling all Worldbit-related debug output
            if(ptrCategory->count(string("worldbit"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\WorldbitCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\WorldbitCore
    // Mac: ~/Library/Application Support/WorldbitCore
    // Unix: ~/.worldbitcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WorldbitCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/WorldbitCore";
#else
    // Unix
    return pathRet / ".worldbitcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty worldbit.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override worldbit.conf
