 ...
 *
 *   // network
 *   CWorldbitNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.worldbit.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CWorldbitNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CWorldbitNetwork(UniValue objIn)
    {
...
// // can be under: WorldbitNetwork
// //   -- signature requirements : Key1(User)
// class CWorldbitNetworkVariable : public CGovernanceObject
// {
// private:
